...
void Pong::game_loop() {
    ...
    while (keep_running) {
        while (SDL_PollEvent(&m_game_window_event) > 0) {
            ...
            m_left_paddle.handle_input(m_game_window_event); // Keybinds
            m_right_paddle.handle_input(m_game_window_event); // Keybinds
        }
...
void Pong::update(double delta_time) {
    ...
    m_ball.update(delta_time, m_game_window_renderer, 75, m_left_paddle.getWH(), m_right_paddle.getWH());
}
